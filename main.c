...
#define REQUIRED_LANGDLL_VERSION  22UL
...
int main(int argc, char **argv)
{
   ...
   if (!(anchor=WinInitialize(0)))
     return RET_ERROR;                      /* Init fehlgeschlagen       */

   if (!(queue=WinCreateMsgQueue(anchor,100)))
     return RET_ERROR;                      /* Queue Init fehlgeschlagen */
   ...
   /* Menue laden */
   if (!(hwndmenu=WinLoadMenu(frame, hmodLang, IDM_MENU)))
      return RET_ERROR;
